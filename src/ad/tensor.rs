...
impl ADTensor {
    pub fn new_input(value: NdarrayStorage, tape_id: usize) -> Self {
        let node = ADNode::TensorInput { value: value.clone() };
    ...
    pub fn eval(&self) -> NdarrayStorage {
        match self {
            ...
            ADTensor::Dual { tape_id, node_id, value } => {
                if let Some(v) = value {
                    return v.clone();
                }
                with_tape(*tape_id, |tape| {
                    evaluate_forward(tape, *node_id)
                })
            }
        }
    }
...
fn push_tensor_binary(lhs: &ADTensor, rhs: &ADTensor, op: BinaryOp) -> ADTensor {
    match (lhs, rhs) {
        (ADTensor::Dual { tape_id: t1, node_id: n1, .. }, ADTensor::Dual { tape_id: t2, node_id: n2, .. }) => {
...
fn check_value(node: &ADNode) -> Option<&NdarrayStorage> {
    match node {
        ...
        ADNode::TensorBinary { value, .. } | ADNode::TensorUnary { value, .. } | ADNode::TensorFusedMulAdd { value, .. } => value.as_ref(),
        _ => None,
    }
}

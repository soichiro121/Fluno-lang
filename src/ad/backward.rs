        ...
        match &nodes[node_id] {
            ...
            ADNode::Binary {
                ...
                if let ADGradient::Scalar(g) = grad {
                    ...
                    let (grad_lhs, grad_rhs) =
                        compute_binary_grad(*op, lhs_val, rhs_val, *value, g);
            ...
            ADNode::TensorBinary {
                ...
                if let ADGradient::Tensor(g) = grad {
                    ...
                    let (grad_lhs, grad_rhs) =
                        compute_tensor_binary_grad(*op, lhs_val, rhs_val, &g);
            ...
            ADNode::CustomVjp { name, args, value } => {
                if let ADGradient::Scalar(g) = grad {
                    let input_vals: Vec<f64> =
                        args.iter().map(|id| get_node_scalar(&nodes, *id)).collect();
...
fn get_node_tensor(nodes: &[ADNode], id: usize) -> &NdarrayStorage {
    match &nodes[id] {
        ...
        | ADNode::TensorFusedMulAdd { value, .. } => value
            .as_ref()
            .expect("Internal Error: Tensor value not evaluated during backward pass"),
...
fn compute_unary_grad(op: UnaryOp, input_val: f64, output_val: f64, grad_output: f64) -> f64 {
    match op {
        ...
        UnaryOp::Sqrt => grad_output / (2.0 * output_val),
        UnaryOp::Abs => {
            if input_val > 0.0 {
                grad_output
        ...
        UnaryOp::Tanh => (1.0 - output_val * output_val) * grad_output,
        UnaryOp::Sigmoid => output_val * (1.0 - output_val) * grad_output,
        UnaryOp::LGamma => {
...
fn compute_binary_grad(
    ...
    match op {
        ...
        BinaryOp::Mul => (rhs_val * grad_output, lhs_val * grad_output),
        BinaryOp::Div => (
            grad_output / rhs_val,
            -lhs_val * grad_output / (rhs_val * rhs_val),
        ),
        BinaryOp::Pow => {
        ...
        BinaryOp::Atan2 => {
            ...
            (
                rhs_val / denom * grad_output,
                -lhs_val / denom * grad_output,
            )
        }
        BinaryOp::BetaSample => {
...
fn compute_tensor_unary_grad(
    ...
    for ((gin, &vin), (&vout, &gout)) in grad_input
        .iter_mut()
        .zip(input_val.0.iter())
    ...
    match op {
        ...
        BinaryOp::MatMul => {
            let lhs_2d = lhs
                ...
                .expect("MatMul lhs must be 2D");
            let rhs_2d = rhs
                .0
                .view()
                .into_dimensionality::<ndarray::Ix2>()
                .expect("MatMul rhs must be 2D");
            let grad_2d = grad_output
                .0
                .view()
                .into_dimensionality::<ndarray::Ix2>()
                .expect("MatMul grad must be 2D");
...
mod tests {
    ...
    fn test_simple_gradient() {
        ...
        {
            let grads = crate::ad::with_tape(tape_id, |tape| backward(tape, node_id));

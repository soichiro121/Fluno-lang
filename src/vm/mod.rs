    ...
    pub fn eval_expression(&mut self, expr: &Expression) -> RuntimeResult<Value> {
        match expr {
            ...
            Expression::TryMethodCall {
                ...
                if should_return {
                    return Ok(actual_receiver);
                }

                // Temporary fallback for AST interpreter. AST interpreter does not currently fully support ?. syntax.
                // It requires extensive refactoring of MethodCall block (Phase 2).
                Err(RuntimeError::Unimplemented("TryMethodCall is only partially implemented in the AST interpreter. Please compile to bytecode.".into()))
            }

...
impl Interpreter {
    ...
    pub fn run_inference(&mut self, num_samples: usize, model_func: Value) -> RuntimeResult<Value> {
        ...
        match samples_result {
            Ok(samples) => {
                ...
                Ok(Value::Array(crate::gc::Rc::new(RefCell::new(val_samples))))
            },
            Err(e) => Err(e),
        }
    }

    fn _make_sample(&self, value: Value, log_weight: f64) -> Value {
        use std::collections::HashMap;
        use crate::gc::Rc;
    ...
    fn to_gaussian_if_struct(&self, val: &Value) -> Value {
        if let Value::Struct { name, fields } = val {
            if name == "Gaussian" {
                let mean = fields.borrow().get("mean").and_then(|v| match v { Value::Float(f) => Some(f.clone()), _ => None });
                let std = fields.borrow().get("std").and_then(|v| match v { Value::Float(f) => Some(f.clone()), _ => None });
                    ...
                    if let Some(Type::Named { name, .. }) = ty {
                        ...
                        if let Type::DynTrait { trait_path: ref traitpath } = **elem_ty {
                            ...
                            if let Value::Array(ref elements) = val {
                                ...
                                val = Value::Array(Rc::new(RefCell::new(converted)));
                            }
                        }
                    }
            ...
            Statement::For { pattern, iterator, body, .. } => {
                ...
                match iter_val {
                    Value::Array(elements) => {
                        let elements_borrow = elements.borrow();
                        for element in elements_borrow.iter() {
    ...
    pub fn eval_expression(&mut self, expr: &Expression) -> RuntimeResult<Value> {
        match expr {
                                ...
                                if let Expression::Variable { name: var_name, .. } = object.as_ref() {
                                    ...
                                    match obj_val {
                                        Value::Struct { name: struct_name, fields } => {
                                            if fields.borrow().contains_key(&field.name) {
                                                fields.borrow_mut().insert(field.name.clone(), new_val.clone());
                                                return Ok(new_val);
                                            } else {
                                                return Err(RuntimeError::TypeMismatch {
                                                    message: format!(
                                ...
                                if let Expression::Variable { name: var_name, .. } = object.as_ref() {
                                    ...
                                    match (arr_val, idx_val) {
                                        (Value::Array(elements), Value::Int(idx)) => {
                                            ...
                                            if idx_usize >= elements.borrow().len() {
                                                return Err(RuntimeError::TypeMismatch {
                                                    message: format!(
                                                        "Index {} out of bounds for array of length {}",
                                                        idx, elements.borrow().len()
                                                    ),
                                                });
                                            }
                                            
                                            elements.borrow_mut()[idx_usize] = new_val.clone();
                                            return Ok(new_val);
                                        }
                                        _ => {
                                            return Err(RuntimeError::TypeMismatch {
            ...
            Expression::UfcsMethod { trait_path, method, span: _span } => {
                return Err(RuntimeError::InvalidOperation {
            ...
            Expression::Spawn { expr, .. } => {
                ...
                let result_slot: Arc<Mutex<Option<Result<Value, String>>>> = Arc::new(Mutex::new(None));
                ...
                Ok(Value::Future { task_id, result: result_slot })
            }
            Expression::Await { expr, span: _span } => {
                ...
                match future_val {
                    ...
                    _ => Err(RuntimeError::TypeMismatch {
                        message: format!("'await' requires a Future, got {}", future_val.type_name()),
                    }),
                }
            }
            ...
            Expression::Range { start, end, inclusive, span: _span } => {
                ...
                Ok(Value::Range { start: start_val, end: end_val, inclusive: *inclusive })
            }

            Expression::Cast { expr, target_type, span: _span } => {
                ...
                match (val.clone(), target_type) {
                    ...
                    (Value::Int(i), Type::String) => Ok(Value::String(crate::gc::Rc::new(i.to_string()))),
                    (Value::Float(f), Type::String) => Ok(Value::String(crate::gc::Rc::new(f.value().to_string()))),
                    (Value::Bool(b), Type::String) => Ok(Value::String(crate::gc::Rc::new(b.to_string()))),
                    _ => Ok(val),
                }
            }
        ...
        match name {
            ...
            "Signal_filter" => {
                ...
                let new_closure = if let Value::Function { params, body, closure, .. } = closure_val {
                    ...
                    Value::Function { params, body, closure: Rc::new(new_env), is_async: false }
                } else {
            ...
            "Array::push" => {
                ...
                match &args[0] {
                    Value::Array(arr) => {
                        ...
                        new_arr.borrow_mut().push(args[1].clone());
            ...
            "infer_hmc" => {
                ...
                match samples_result {
                    Ok(samples) => {
                        ...
                        Ok(Value::Array(Rc::new(RefCell::new(map_samples))))
                    },
                    Err(e) => Err(e),
                }
            }
    ...
    fn eval_call(&mut self, callee: &Expression, args: &[Expression], span: Span) -> RuntimeResult<Value> {
        ...
        if let Expression::UfcsMethod { trait_path: _, method, .. } = callee {
            ...
            if let (Value::Array(arr), "head") = (&arg_values[0], method.name.as_str()) {
                ...
                let elements: &Vec<Value> = &*borrowed;
    ...
    fn eval_call_value(&mut self, func_val: Value, arg_values: &[Value], span: Span) -> RuntimeResult<Value> {
        match func_val {
            ...
            Value::Function { params, body, closure, is_async } => {
                ...
                let execute_fn = |interp: &mut Interpreter, params: &[Parameter], body: &Block, closure: &Rc<Environment>, arg_values: &[Value]| -> RuntimeResult<Value> {
                    ...
                    let result = match interp.eval_block(&body) {
                        Ok(val) => val,
                        Err(RuntimeError::EarlyReturn) => interp.return_value.take().unwrap_or(Value::Unit),
                        Err(e) => {
                            interp.pop_scope();
            ...
            Value::NativeFunction { name, library_index, params, return_type, is_async } => {
                if is_async {
                    ...
                    let res = self.call_native(&name, library_index, &params, &return_type, arg_values.to_vec(), span);
                    {
                        let mut lock = result_slot.lock().unwrap();
    ...
    fn eval_index(&self, object: &Value, index: &Value) -> RuntimeResult<Value> {
        match (object, index) {
            (Value::Array(elements), Value::Int(idx)) => {
                ...
                borrowed.get(i).cloned().ok_or_else(|| RuntimeError::IndexOutOfBounds { index: i, length: borrowed.len() })
            },
    ...
    fn pattern_matches(&self, pattern: &Pattern, value: &Value) -> RuntimeResult<bool> {
        match (pattern, value) {
            ...
            (Pattern::Tuple { patterns, .. }, Value::Array(values)) => {
                ...
                if patterns.len() != values_borrow.len() { return Ok(false); }
            ...
            (Pattern::Struct { name, fields, .. }, value) => {
                if let Value::Struct { name: s_name, fields: s_fields } = value {
                    ...
                    for field_pat in fields {
                        ...
                        if let Some(val) = s_fields.borrow().get(field_name) {
        ...
        match pattern {
            ...
            Pattern::Tuple { patterns, .. } => {
                if let Value::Tuple(values) = value {
                    ...
                    for (pat, val) in patterns.iter().zip(values.borrow().iter()) { self.bind_pattern(pat, val.clone())?; }
                    Ok(())
            ...
            Pattern::Struct { name, fields, .. } => {
                if let Value::Struct { name: s_name, fields: s_fields } = value {
                    ...
                    for field_pat in fields {
                        let field_name = &field_pat.name.name;
                        let val = s_fields.borrow().get(field_name).cloned().ok_or_else(|| RuntimeError::TypeMismatch {
...
fn value_to_json(v: &Value) -> serde_json::Value {
    match v {
        ...
        Value::Array(arr) => serde_json::Value::Array(arr.borrow().iter().map(|item| value_to_json(item)).collect()),
        Value::Map(m) => {
...
fn json_to_value(v: &serde_json::Value) -> Value {
    match v {
        ...
        serde_json::Value::Array(arr) => {
            Value::Array(crate::gc::Rc::new(std::cell::RefCell::new(arr.iter().map(|item| json_to_value(item)).collect())))
        },
        serde_json::Value::Object(map) => {
            if let Some(serde_json::Value::String(ty)) = map.get("__type") {
                ...
                Value::Struct { name: ty.clone(), fields: crate::gc::Rc::new(std::cell::RefCell::new(fields)) }

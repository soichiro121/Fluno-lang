...
use crate::ast::node::{
    Block, Expression, ImplItem, Item, Literal, Pattern, Program, Span, Statement, Type,
};
...
impl CodeGenerator {
        ...
        match item {
            Item::Function(func) => {
                ...
                for (i, param) in func.params.iter().enumerate() {
                    ...
                    self.type_env
                        .insert(param.name.name.clone(), param.ty.clone());
                }
                self.push(") ");
            ...
            Item::Extern(ext) => {
                ...
                for f in &ext.functions {
                    if f.is_async {
                        self.indent();
                        self.push_line(
                            "// NOTE: Async extern fn is not directly supported in Rust FFI.",
                        );
                        self.indent();
    ...
    fn gen_type(&mut self, ty: &Type) -> FlunoResult<()> {
        match ty {
            ...
            Type::VonMises => self.push("VonMises"),
        ...
        match stmt {
            Statement::Let {
                ...
                if let Pattern::Identifier { name, .. } = pattern {
                    let var_type = ty
                        .clone()
                        .unwrap_or_else(|| Self::infer_type_from_expr(init.as_ref()));
            ...
            _ => {
                return Err(Error::CompilationError(
                    "Statement not supported yet".to_string(),
                ))
            }
        }
        Ok(())

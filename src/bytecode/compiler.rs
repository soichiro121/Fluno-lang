    ...
    fn compile_expression(&mut self, expr: &Expression) -> CompileResult<()> {
        ...
        match expr {
            ...
            Expression::TryMethodCall {
                ...
                let method_idx = self.make_constant(Value::String(Rc::new(method.name.clone())))?;
                let mut instr = Instruction::with_u16(Opcode::TryMethodCall, method_idx, line);
                instr.operands.push(args.len() as u8);
                self.emit(instr);
                Ok(())
            }
            ...
            Expression::Cast {
                ...
                self.compile_expression(expr)?;
                if let crate::ast::node::Type::DynTrait { trait_path } = target_type {
                    let trait_name = trait_path.to_string();
                    let trait_idx = self.make_constant(Value::String(Rc::new(trait_name)))?;
                    self.emit(Instruction::with_u16(Opcode::MakeTraitObject, trait_idx, line));
                } else {

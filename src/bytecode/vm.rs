    ...
    fn run(&mut self, target_depth: usize) -> VMResult<Value> {
        loop {
            ...
            match instr.opcode {
                ...
                Opcode::TryMethodCall => {
                    let method_name_idx = u16::from_le_bytes([instr.operands[0], instr.operands[1]]);
                    let arg_count = instr.operands[2] as usize;
                    ...
                    let mut is_early_return = false;
                    let mut early_return_value = Value::Unit;
                    
                    let actual_receiver = match receiver {
                        Value::Enum { ref name, ref variant, ref fields } if name == "Option" => {
                            if variant == "None" {
                                is_early_return = true;
                                early_return_value = Value::Enum {
                                    name: "Option".to_string(),
                                    variant: "None".to_string(),
                                    fields: Rc::new(Vec::new()),
                                };
                                Value::Unit
                            } else if variant == "Some" && !fields.is_empty() {
                                fields[0].clone()
                            } else {
                                receiver.clone()
                            }
                        ...
                        Value::Enum { ref name, ref variant, ref fields } if name == "Result" => {
                            if variant == "Err" {
                                is_early_return = true;
                                early_return_value = receiver.clone();
                                Value::Unit
                            } else if variant == "Ok" && !fields.is_empty() {
                                fields[0].clone()
                            } else {
                                receiver.clone()
                            }
                    ...
                    if is_early_return {
                        ...
                        self.push(early_return_value)?;
                        continue;
                    }

                    // Replace the receiver on the stack with the unwrapped value
                    let rec_idx = self.stack.len() - 1 - arg_count;
                    self.stack[rec_idx] = actual_receiver.clone();

                    let type_name = match &actual_receiver {
                        Value::Int(_) => "Int".to_string(),
                        Value::Float(_) => "Float".to_string(),
                        Value::Bool(_) => "Bool".to_string(),
                        Value::String(_) => "String".to_string(),
                        ...
                        Value::Bernoulli { .. } => "Bernoulli".to_string(),
                        Value::Beta { .. } => "Beta".to_string(),
                        _ => return Err(VMError::TypeError(format!("Method call not supported on {:?}", actual_receiver))),
                    };
                ...
                Opcode::MakeTraitObject => {
                    let trait_idx = u16::from_le_bytes([instr.operands[0], instr.operands[1]]);
                    let trait_name = match &self.chunks[chunk_index].constants[trait_idx as usize] {
                        Value::String(s) => s.to_string(),
                        _ => return Err(VMError::InvalidOperation("Invalid trait name".into())),
                    };
                    
                    let data = self.pop()?;
                    let vtable = HashMap::new(); 
                    
                    self.push(Value::TraitObject {
                        trait_name,

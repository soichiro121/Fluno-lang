...
impl From<crate::vm::RuntimeError> for VMError {
    fn from(err: crate::vm::RuntimeError) -> Self {
        ...
        match err {
            ...
            RuntimeError::ArityMismatch { expected, found, .. } => VMError::ArityMismatch { expected, found },
...
use crate::vm::prob_runtime::{ProbContext, InferenceMode, Optimizer, Adam, SGD};
...
impl BytecodeVM {
    pub fn new() -> Self {
        ...
        vm.globals.insert("infer".to_string(), Value::Builtin("infer".to_string()));
        vm.globals.insert("infer_hmc".to_string(), Value::Builtin("infer_hmc".to_string())); // Added infer_hmc
        vm.globals.insert("len".to_string(), Value::Builtin("len".to_string()));
        vm.globals.insert("gaussian".to_string(), Value::Builtin("gaussian".to_string()));
         vm.globals.insert("Gaussian".to_string(), Value::Builtin("gaussian".to_string())); // Alias
        vm.globals.insert("uniform".to_string(), Value::Builtin("uniform".to_string()));
        vm.globals.insert("bernoulli".to_string(), Value::Builtin("bernoulli".to_string()));
        vm.globals.insert("beta".to_string(), Value::Builtin("beta".to_string()));
        vm.globals.insert("sample".to_string(), Value::Builtin("sample".to_string())); // Added sample (commonly used as wrapper)
        vm.globals.insert("observe".to_string(), Value::Builtin("observe".to_string())); // Added observe
        vm.globals.insert("Map".to_string(), Value::Builtin("Map".to_string())); // Added Map constructor
        vm.globals.insert("infer_vi".to_string(), Value::Builtin("infer_vi".to_string()));
        vm.globals.insert("param".to_string(), Value::Builtin("param".to_string()));
        vm.globals.insert("println".to_string(), Value::Builtin("println".to_string()));
        vm.globals.insert("sin".to_string(), Value::Builtin("sin".to_string()));
        vm.globals.insert("cos".to_string(), Value::Builtin("cos".to_string()));
        vm.globals.insert("tan".to_string(), Value::Builtin("tan".to_string()));
        vm.globals.insert("exp".to_string(), Value::Builtin("exp".to_string()));
        vm.globals.insert("ln".to_string(), Value::Builtin("ln".to_string()));
        vm.globals.insert("sqrt".to_string(), Value::Builtin("sqrt".to_string()));
        vm.globals.insert("pow".to_string(), Value::Builtin("pow".to_string()));
        
        vm.globals.insert("Map::insert".to_string(), Value::Builtin("Map::insert".to_string()));
        vm.globals.insert("Map::get".to_string(), Value::Builtin("Map::get".to_string()));
        
        vm.globals.insert("Array::push".to_string(), Value::Builtin("Array::push".to_string()));
        vm.globals.insert("Signal_new".to_string(), Value::Builtin("Signal_new".to_string()));
        vm.globals.insert("Signal_get".to_string(), Value::Builtin("Signal_get".to_string()));
        vm.globals.insert("grad".to_string(), Value::Builtin("grad".to_string()));
        vm.globals.insert("backward".to_string(), Value::Builtin("backward".to_string()));
        
        vm.globals.insert("Signal_set".to_string(), Value::Builtin("Signal_set".to_string()));
        ...
        loop {
            ...
            match instr.opcode {
                ...
                Opcode::Index => {
                    ...
                    match (&obj, &index) {
                        (Value::Array(arr), Value::Int(i)) => {
                            ...
                            self.push(arr_borrow[idx].clone())?;
                        }
                ...
                Opcode::Iterator => {
                    ...
                    let iter_state = match val {
                        Value::Range { start, end, inclusive } => {
                            ...
                            IteratorState::Range { current: start, end, inclusive }
                        }
                        Value::Array(elements) => {
                            use crate::vm::value::IteratorState;
                            IteratorState::Array { array: elements, index: 0 }
                        }
                        _ => return Err(VMError::TypeError(format!("Cannot iterate over {:?}", val))),
                    };
                ...
                Opcode::Next => {
                   ...
                   let next_val = match iter_val {
                       Value::Iterator(state_rc) => {
                            ...
                            match *state {
                                IteratorState::Range { ref mut current, end, inclusive } => {
                                ...
                                IteratorState::Array { ref array, ref mut index } => {
                                    if *index < array.borrow().len() {
                                        let val = array.borrow()[*index].clone();
                                        *index += 1;
                                        Some(val)
                                    } else {
                                        None
                       ...
                       _ => return Err(VMError::InvalidOperation("Next called on non-iterator".into())),
                   };
                ...
                Opcode::MakeGaussian => {
                    ...
                    self.push(Value::Gaussian { mean: mean_f, std: std_f })?;
                }
                ...
                Opcode::MakeUniform => {
                    ...
                    self.push(Value::Uniform { min: min_f, max: max_f })?;
                }
                ...
                Opcode::Closure => {
                    ...
                    match prototype {
                        Value::BytecodeFunction { upvalue_count, .. } => {
                            ...
                            for _ in 0..upvalue_count {
                                if offset + 1 >= instr.operands.len() {
                                    return Err(VMError::InvalidOperation("Malformed closure instruction".into()));
                                }
                                ...
                                if is_local {
                                    ...
                                    if let Value::BytecodeClosure { upvalues: ref current_upvalues, .. } = self.frame().closure {
                                        if index >= current_upvalues.len() {
                                            return Err(VMError::IndexOutOfBounds);
                                        }
                                        upvalues.push(current_upvalues[index].clone());
                                    } else {
                                        return Err(VMError::InvalidOperation("Closure instruction in non-closure frame".into()));
                                    }
                                }
                            }
                        ...
                        _ => return Err(VMError::InvalidOperation("Closure operand must be a function".into())),
                    }
                }
                ...
                Opcode::GetUpvalue => {
                    ...
                    let upvalue = if let Value::BytecodeClosure { ref upvalues, .. } = self.frame().closure {
                        upvalues[slot].clone()
                    } else {
                        return Err(VMError::InvalidOperation("GetUpvalue in non-closure frame".into()));
                    };
                ...
                Opcode::SetUpvalue => {
                    ...
                    let upvalue = if let Value::BytecodeClosure { ref upvalues, .. } = self.frame().closure {
                        ...
                        return Err(VMError::InvalidOperation("SetUpvalue in non-closure frame".into()));
                    };
                ...
                Opcode::MakeBeta => {
                    ...
                    self.push(Value::Beta { alpha: alpha_f, beta: beta_f })?;
                }
                        ...
                        match mode {
                            ...
                            InferenceMode::Gradient { tape_id } => {
                                let trace_val = ctx.trace.get(&key).cloned().ok_or(
                                    VMError::InvalidOperation("Missing trace in Gradient mode".into())
                                )?;
                                ...
                                let val_node = match trace_val {
                                    ...
                                    _ => return Err(VMError::TypeError("Sample must be float in gradient mode".into()))
                                };
                            ...
                            InferenceMode::Guide { tape_id: _ } => {
                                if let Some(trace_val) = ctx.trace.get(&key).cloned() {
                                    let val_ad = match &trace_val {
                                        ...
                                        _ => return Err(VMError::TypeError("Replay value must be number".into())),
                                    };
                                    ...
                                    use crate::vm::prob_runtime::{get_distribution_sample, get_distribution_log_pdf_ad};
                                     
                                    let val_ad = get_distribution_sample(&dist, &mut self.rng, mode)?;
                ...
                Opcode::DefineMethod => {
                    ...
                    let method_name_idx = u16::from_le_bytes([instr.operands[2], instr.operands[3]]);
                    
                    let type_name = match &self.chunks[chunk_index].constants[type_name_idx as usize] {
                        Value::String(s) => s.to_string(),
                        _ => return Err(VMError::InvalidOperation("Invalid type name".into())),
                    };
                    let method_name = match &self.chunks[chunk_index].constants[method_name_idx as usize] {
                        Value::String(s) => s.to_string(),
                        _ => return Err(VMError::InvalidOperation("Invalid method name".into())),
                    };
                    
                    let func = self.pop()?;
                    self.methods.entry(type_name).or_default().insert(method_name, func);
                }
                ...
                Opcode::MakeStruct => {
                    ...
                    for _ in 0..field_count {
                        ...
                        let key = match key_val {
                            Value::String(s) => s.to_string(),
                            _ => return Err(VMError::TypeError("Struct key must be string".into())),
                        };
                ...
                Opcode::GetField => {
                    ...
                    match obj {
                        Value::Struct { fields, .. } => {
                            match fields.borrow().get(&field_name) {
                                Some(val) => self.push(val.clone())?,
                                None => return Err(VMError::UndefinedField(format!("Field '{}' not found", field_name))),
                            }
                        }
                        _ => return Err(VMError::TypeError("Expected struct for field access".into())),
                    }
                }
                ...
                Opcode::UnpackEnum => {
                    ...
                    match obj {
                        ...
                        _ => return Err(VMError::TypeError(format!("UnpackEnum on non-enum: {}", obj.type_name()).into())),
                    }
                }
                ...
                Opcode::MethodCall => {
                    ...
                    match (&receiver, method_name.as_str()) {
                        ...
                        (Value::Map(map_rc), "insert") => {
                            if arg_count != 2 {
                                return Err(VMError::ArityMismatch { expected: 2, found: arg_count });
                            }
                            let value = self.pop()?;
                            let key_val = self.pop()?;
                            let key = match key_val {
                        ...
                        (Value::Map(map_rc), "get") => {
                            if arg_count != 1 {
                                return Err(VMError::ArityMismatch { expected: 1, found: arg_count });
                            }
                            let key_val = self.pop()?;
                            let key = match key_val {
                                Value::String(s) => s.to_string(),
                            ...
                            let val = map.get(&key).cloned().unwrap_or(Value::Unit);
                            self.push(val)?;
                            continue;
                        }
                        _ => {}
                    ...
                    let type_name = match &receiver {
                        ...
                        _ => return Err(VMError::TypeError(format!("Method call not supported on {:?}", receiver))),
                    };
                    ...
                    if let Some(m) = method {
                        ...
                        return Err(VMError::InvalidOperation(format!("Method '{}' not found for type '{}'", method_name, type_name)));
                    }
                }
    ...
    fn binary_add(&mut self) -> VMResult<()> {
        ...
        match (&a, &b) {
            (Value::Gaussian { mean: m1, std: s1 }, Value::Gaussian { mean: m2, std: s2 }) => {
                ...
                self.push(Value::Gaussian { mean: new_mean, std: new_std })
            }
            (Value::Int(x), Value::Int(y)) => {
                self.push(Value::Int(x + y))
            }
    ...
    fn binary_sub(&mut self) -> VMResult<()> {
        ...
        match (&a, &b) {
            ...
            _ => Err(VMError::TypeError(format!("Cannot subtract {} by {}", a.type_name(), b.type_name()))),
        }
    }
    ...
    fn binary_mul(&mut self) -> VMResult<()> {
        ...
        match (&a, &b) {
            ...
            _ => Err(VMError::TypeError(format!("Cannot multiply {} and {}", a.type_name(), b.type_name()))),
        }
    }
    ...
    fn binary_div(&mut self) -> VMResult<()> {
        ...
        match (&a, &b) {
            ...
            _ => Err(VMError::TypeError(format!("Cannot divide {} by {}", a.type_name(), b.type_name()))),
        }
    }
    ...
    fn call_value(&mut self, arg_count: usize) -> VMResult<()> {
        ...
        match callee {
            Value::BytecodeFunction { chunk_index, arity, .. } => {
                if arity != arg_count {
            ...
            Value::BytecodeClosure { ref function, upvalues: _ } => {
                if let Value::BytecodeFunction { chunk_index, arity, .. } = **function {
                    if arity != arg_count {
                        return Err(VMError::ArityMismatch {
                            expected: arity,
                            found: arg_count,
                    ...
                    Err(VMError::InvalidOperation("Closure does not contain a function".into()))
                }
            }
            Value::Function {   .. } => {
    ...
    fn call_builtin(&mut self, name: &str, args: Vec<Value>) -> VMResult<Value> {
        match name {
            "infer" => {
                if args.len() != 2 {
                    return Err(VMError::ArityMismatch { expected: 2, found: args.len() });
                }
                
                let num_samples = match args[0] {
                    Value::Int(n) => n as usize,
                    _ => return Err(VMError::TypeError("infer arg 1 must be Int (num_samples)".into())),
                };
            ...
            "len" => {
                if args.len() != 1 {
                    return Err(VMError::ArityMismatch { expected: 1, found: args.len() });
                }
                match &args[0] {
            ...
            "param" => {
                if args.len() != 2 { return Err(VMError::ArityMismatch { expected: 2, found: args.len() }); }
            ...
            "Map" => {
                if args.len() != 0 {
                    return Err(VMError::ArityMismatch { expected: 0, found: args.len() });
                }
            ...
            "sin" => {
                if args.len() != 1 { return Err(VMError::ArityMismatch { expected: 1, found: args.len() }); }
            ...
            "Array::push" => {
                ...
                let new_arr = if let Value::Array(rc_vec) = &args[0] {
                    let mut new_vec = rc_vec.borrow().clone();
            ...
            "Signal_new" => {
                ...
                let id = self.reactive_runtime.create_root(initial_value.clone(), crate::vm::reactive::NodeKind::Signal);
                Ok(Value::Signal { id, current_value: Rc::new(initial_value) }) 
            }
            "Signal_get" => {
                if args.len() != 1 { return Err(VMError::ArityMismatch { expected: 1, found: args.len() }); }
                if let Value::Signal { id, .. } = &args[0] {
                    let val = self.reactive_runtime.get_value(*id)
            ...
            "infer_hmc" => {
                if args.len() != 5 {
                    return Err(VMError::ArityMismatch { expected: 5, found: args.len() });
                }
                
                let num_samples = match &args[1] { Value::Int(i) => *i as usize, _ => return Err(VMError::TypeError("Argument 1 (num_samples) must be Int".into())) };
                let burn_in = match &args[2] { Value::Int(i) => *i as usize, _ => return Err(VMError::TypeError("Argument 2 (burn_in) must be Int".into())) };
                let epsilon = match &args[3] { Value::Float(f) => f.value(), _ => return Err(VMError::TypeError("Argument 3 (epsilon) must be Float".into())) };
                let l_steps = match &args[4] { Value::Int(i) => *i as usize, _ => return Err(VMError::TypeError("Argument 4 (l_steps) must be Int".into())) };
                ...
                for sample in samples {
                    ...
                    samples_val.push(Value::Map(Rc::new(RefCell::new(map))));
                }
                Ok(Value::Array(Rc::new(RefCell::new(samples_val))))
            }
            "infer_vi" => {
                if args.len() != 4 {
                    return Err(VMError::ArityMismatch { expected: 4, found: args.len() });
                }
            ...
            "Map::insert" => {
                if args.len() != 3 { return Err(VMError::ArityMismatch { expected: 3, found: args.len() }); }
                if let Value::Map(map) = &args[0] {
                    ...
                    Err(VMError::TypeError("First argument to Map::insert must be Map".into()))
                }
            }
            "Map::get" => {
                if args.len() != 2 { return Err(VMError::ArityMismatch { expected: 2, found: args.len() }); }
                if let Value::Map(map) = &args[0] {
                    ...
                    let val = map.borrow().get(&key).cloned().unwrap_or(Value::None);
                    Ok(val)
                } else {
                    Err(VMError::TypeError("First argument to Map::get must be Map".into()))
                }
            }
            _ => Err(VMError::InvalidOperation(format!("Unknown builtin: {}", name))),
        }
    }
    ...
    fn run_inference(&mut self, num_samples: usize, model_fn: Value) -> VMResult<Value> {
        let (chunk_index, upvalues_opt) = match &model_fn {
            ...
            Value::BytecodeClosure { function, upvalues } => {
                if let Value::BytecodeFunction { chunk_index, .. } = &**function {
                    ...
                    return Err(VMError::TypeError("infer requires a bytecode function".into()));
                }
            }
        ...
        if samples.is_empty() {
            return Err(VMError::InvalidOperation("No valid samples in inference".into()));
        }
        
        let total_weight: f64 = weights.iter().sum();
        if total_weight <= 0.0 {
            let result: Vec<Value> = samples.iter()
        ...
        Ok(Value::Array(Rc::new(RefCell::new(resampled))))
    }
    ...
    fn run_vi(&mut self, config: Value, model_fn: Value, guide_fn: Value) -> VMResult<Value> {
        let (num_iters, optimizer_name, lr) = if let Value::Map(m) = config {
            let map = m.borrow();
            let n = match map.get("num_iters") { Some(Value::Int(i)) => *i as usize, _ => 1000 };
            let opt = match map.get("optimizer") { Some(Value::String(s)) => s.as_str().to_string(), _ => "adam".to_string() };
            let r = match map.get("learning_rate") { Some(Value::Float(f)) => f.value(), Some(Value::Int(i)) => *i as f64, _ => 0.01 };
            (n, opt, r)
        } else {
        ...
        for _ in 0..num_iters {
            let (grads, param_nodes) = self.run_guide_and_model_vi(&model_fn, &guide_fn, &mut vi_params)?;
    ...
    fn run_guide_and_model_vi(&mut self, model_fn: &Value, guide_fn: &Value, vi_params: &mut HashMap<String, f64>) 
        ...
        match function {
            Value::BytecodeFunction { chunk_index, arity, .. } => {
                self.frames.push(CallFrame {
                    ...
                    closure: Value::BytecodeClosure { function: Box::new(function.clone()), upvalues: vec![] },
                });
            }
            Value::BytecodeClosure { function: func, upvalues: _ } => {
                if let Value::BytecodeFunction { chunk_index, arity, .. } = &**func {
                    self.frames.push(CallFrame {
                        chunk_index: *chunk_index,
                        ip: 0,
                        slot_base: self.stack.len() - arity,
    ...
    fn run_model_and_get_grads(&mut self, model_fn: &Value, q_values: &HashMap<String, f64>) 
        ...
        let slot_base = self.stack.len();
        let closure = if let Some(ref upvals) = upvalues_opt {
            Value::BytecodeClosure { function: Box::new(model_fn.clone()), upvalues: upvals.clone() }
        } else {
            Value::Unit 
        };
    ...
    pub fn run_hmc(&mut self, model: &Value, num_samples: usize, burn_in: usize, epsilon: f64, l_steps: usize) 
...
fn sample_distribution(dist: &Value, rng: &mut rand::rngs::ThreadRng) -> VMResult<Value> {
    match dist {
        ...
        _ => Err(VMError::TypeError(
            format!("Cannot sample from {}", dist.type_name())
        )),
    }
}
...
impl BytecodeVM {
    fn log_prob(&self, dist: &Value, value: &Value) -> VMResult<f64> {
        let x = match value {
            ...
            _ => return Err(VMError::TypeError(
                format!("Cannot compute log probability for {}", value.type_name())
            )),
        };
        ...
        match dist {
            Value::Gaussian { mean, std } => {
                ...
                let log_prob = -0.5 * z * z - std.value().ln() - 0.5 * (2.0 * std::f64::consts::PI).ln();
                Ok(log_prob)
            }
            Value::Uniform { min, max } => {
            ...
            Value::Beta { alpha, beta } => {
                ...
                let a = alpha.value();
                let b = beta.value();
                let log_prob = (a - 1.0) * x.ln() + (b - 1.0) * (1.0 - x).ln()
                    + ln_gamma(a + b) - ln_gamma(a) - ln_gamma(b);
                Ok(log_prob)
            }
            _ => Err(VMError::TypeError(
                format!("Cannot compute log probability for distribution {}", dist.type_name())
            )),
        }
    }

...
use crate::ast::node::{
    BinaryOp, Expression, Identifier, Literal, Path, PathSeg, Precedence, Span, UnaryOp,
};
...
impl<'a> Parser<'a> {
        ...
        loop {
            loop {
                ...
                if self.check(TokenKind::Question) {
                    ...
                    if self.check(TokenKind::Dot) {
                        left = self.parse_try_dot_postfix(left)?;
                        continue;
                    }
    ...
    fn parse_try_dot_postfix(&mut self, left: Expression) -> ParseResult<Expression> {
        let _ = self.advance()?;
        ...
        if self.check(TokenKind::LParen) {
            ...
            return Ok(Expression::TryMethodCall {
                receiver: Box::new(left),
                method: name,
                args,
                span,
        ...
        // if not method call, fallback to Try wrapping a FieldAccess for now
        ...
        match &token.kind {
            ...
            TokenKind::SelfLower => {
                ...
                Ok(Expression::Variable {
                    name: Path {
                        segments: vec![PathSeg::Ident(Identifier::new(
                            "self".to_string(),
                            span.clone(),
                        ))],
                        span: span.clone(),
                        ...
                        if self.check(TokenKind::ColonColon) {
                            ...
                            if islower {
                                ...
                                if self.check(TokenKind::LParen) {
                                    ...
                                    if !self.check(TokenKind::RParen) {
                                        loop {
                                            args.push(self.parse_expression_with_precedence(
                                                Precedence::Lowest,
                                            )?);
                                            if !self.match_any(&[TokenKind::Comma]) {
                                                break;
                            ...
                            if self.check(TokenKind::LBrace) {
                                ...
                                if !self.check(TokenKind::RBrace) {
                                    loop {
                                        ...
                                        self.expect(TokenKind::Colon)?;
                                        let value = self
                                            .parse_expression_with_precedence(Precedence::Lowest)?;
                                        args.push(value);
